...
public class OperadorService {
	...
	public Response getClientes () {
		...
		try {
			...
			List<OperadorVO> clientesBD;
			clientesBD = transactManager.getAllOperadores();
	...
	public Response getClienteById( @PathParam( "id" ) Long id )
	{
		try{
			...
			OperadorVO clienteBuscado = tm.getOperador(id);
	...
	@Path( "{id: \\d+}/miDinero" )
	@Produces({ MediaType.APPLICATION_JSON })
	public Response getMyMoney ( @PathParam( "id" ) Long id )
	{
		try{
			AlohAndesTransactionManager tm = new AlohAndesTransactionManager( getPath( ) );
			Double clienteBuscado = tm.getMymoney(id);
			return Response.status( 200 ).entity(clienteBuscado).build();			
		}
		catch( Exception e ){
			return Response.status( 500 ).entity( doErrorMessage( e ) ).build( );
